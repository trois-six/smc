...
//go:generate go run github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen@v2.4.1 --config=../client.yaml --include-tags="default,$(cat ../swagger_flat.yaml | yq e '.. | select(has("tags")) | .tags[]' | sort -u | grep -Ev 'cli|config|deployment|firewalls|misc|networkInterfaces|nsrpc|objects|proxy|QoS|routing|rules|rule-sets' | paste -sd ',')" ../swagger_flat.yaml
